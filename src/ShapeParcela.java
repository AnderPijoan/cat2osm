...
public class ShapeParcela extends Shape {
	...
	public ShapeParcela(SimpleFeature f, String tipo) {
		...
		// El NUMSYMBOL 4 de las parcelas son las que a pesar de tener datos no
		// se tienen que dibujar
		// como parcelas de carreteras, la parcela rustica que cubre toda la
		// zona urbana y alguna mas
		...
		/*
		 * this.atributos = new ArrayList<ShapeAttribute>(); for (int x = 1; x <
		 * f.getAttributes().size(); x++){ atributos.add(new
		 * ShapeAttribute(f.getFeatureType().getDescriptor(x).getType(),
	 ...
	 * 
	 * @param pos
	 *            posicion que ocupa el poligono en la lista
	...
	/**
	 * Devuelve la lista de ids de ways del poligono en posicion pos
	 * 
	 * @param pos
	 *            posicion que ocupa el poligono en la lista
